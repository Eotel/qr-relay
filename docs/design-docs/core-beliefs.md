...
- `ScanRule` が表現する 6 軸 (`value` / `initial` / `onScan.source` / `onScan.sink` /
  `constraints` / `end`) を伸ばす方が、新 handler を作るより常に優先。
- relay で表現できない novel ロジック (例: 任意 payload の蓄積、複雑な経路解析) に限り
  別 handler を実装する。

**Enforcement**:
- `packages/handlers/src/relay.test.ts` の 19 ケースが、9 プリセット全ての initial /
  scan / metrics / isOver を覆っている。新プリセットを足したら必ずテストも足す。
...
`isOver` は副作用なしの pure function として実装する。server と client は同一の
handler を import する。
